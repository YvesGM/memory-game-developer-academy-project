...
import type { PlayerId } from "../../settings/game-setting-types";

// ## TS - FUNCTION-IMPORTS
import { getEndScreenAssets } from "../end-screen-assets";
...
function renderWinnerResult(
  translation: Translations,
  gameState: GameState,
  assets: ReturnType<typeof getEndScreenAssets>
): string {
  ...
  return renderWinnerScreen(
    ...
    gameState,
    winner,
    getWinnerImagePath(winner, assets),
...
function renderResultImage(
  ...
  return `
    <div class="end-screen__result-image-wrapper">
      <img
        class="end-screen__result-icon ${modifierClass}"
        src="${imagePath}"
        alt=""
        aria-hidden="true"
      >
