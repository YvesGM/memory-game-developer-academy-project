...
function renderMemoryCard(
  ...
  return `
    <button
      class="memory-card ${getCardStateClass(card)}"
      type="button"
      data-card-id="${card.id}"
      aria-label="${getCardAriaLabel(translation, card)}"
      aria-pressed="${card.status !== "hidden"}"
      ${getDisabledAttribute(card)}
    >
 ...
 */
function getVisibleCardAriaLabel(
  translation: Translations,
  card: MemoryCard
): string {
  return card.status === "flipped"
    ? translation.game.flippedCardLabel
    : translation.game.hiddenCardLabel;
}