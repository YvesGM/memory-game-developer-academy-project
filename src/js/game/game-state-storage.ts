...
function parseGameState(storedState: string, settings: GameSettings): GameState {
    try {
        ...
        return isValidGameState(parsedState, settings) ? normalizeGameState(parsedState) : createInitialGameState(settings);
    } catch {
        return createInitialGameState(settings);
    }
}
...
function hasValidBaseState(value: Record<string, unknown>, settings: GameSettings): boolean {
    return isGameTheme(value.theme) && isBoardSize(value.boardSize) && isPlayerId(value.activePlayer) && value.theme === settings.theme && value.boardSize === settings.boardSize;
}
...
function hasValidCards(value: unknown, settings: GameSettings): value is MemoryCard[] {
    ...
    return value.length === expectedCount && value.every(isMemoryCard) && hasUniqueCardIds(value);
}
...
function hasValidSelectedCards(value: Record<string, unknown>): boolean {
    ...
    return value.selectedCardIds.every((cardId) => typeof cardId === "string");
}

function normalizeGameState(state: GameState): GameState {
    const selectedCardIds = getStableSelectedIds(state);

    return { ...state, cards: normalizeCards(state.cards, selectedCardIds), selectedCardIds, isBoardLocked: false };
}
...
function normalizeCard(card: MemoryCard, selectedIds: Set<string>): MemoryCard {
    ...
    return {
        ...card, status: selectedIds.has(card.id) ? "flipped" : "hidden"
    };
}
