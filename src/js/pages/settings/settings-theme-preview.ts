...
import type { GameSettings, ThemePreviewConfig } from "../../lib/pages/settings/settings-interfaces";
import type { PlayerId } from "../../lib/pages/settings/settings-types";

import { getPlayerIconPath, getThemePreviewConfig } from "../../lib/pages/settings/settings-theme-preview-config";
...
function renderCurrentPlayer(translation: Translations, playerIcon: string): string {
  return `
    <div class="theme-preview__current-player">
      <span class="theme-preview__current-player-label">${translation.settings.currentPlayer}</span>
...
function renderLabeledScore(basePath: string, player: PlayerId, label: string, score: number): string {
  return `
    <span class="theme-preview__score theme-preview__score--${player}">
      <img class="theme-preview__score-icon" src="${basePath}/${player}-player-icon.svg" alt="">
      <span class="theme-preview__score-label">${label}</span>
      <span class="theme-preview__score-number">${score}</span>
    </span>
  `;
}
