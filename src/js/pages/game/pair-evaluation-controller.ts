// ------------ IMPORTS ------------//
import { CARD_FLIP_BACK_DELAY, CARD_FLIP_DURATION, MAX_SELECTED_CARDS } from "../../lib/pages/game/game-constants";
...
import { addPointToActivePlayer, getSelectedCards, hideSelectedCards, isGameFinished, markSelectedCardsAsMatched, unlockBoard } from "./game-state";
...
export function evaluateSelectedCards(): void {
    const selectedCards = getSelectedCards(appContext.currentGameState);
    if (selectedCards.length !== MAX_SELECTED_CARDS) {
        return;
    }

    const [firstCard, secondCard] = selectedCards;
    processSelectedPair(firstCard.pairId === secondCard.pairId);
}
...
function applyMatchedPair(): void {
    appContext.currentGameState = addPointToActivePlayer(appContext.currentGameState);
...
function markCurrentCardsAsMatched(): void {
    appContext.currentGameState = markSelectedCardsAsMatched(appContext.currentGameState);
}
...
function handleMismatchedPair(): void {
    const selectedIds = [...appContext.currentGameState.selectedCardIds];
...
function hideCurrentPair(): void {
    appContext.currentGameState = hideSelectedCards(appContext.currentGameState);
}
...
function finishMismatchedPair(): void {
    appContext.currentGameState = unlockBoard(appContext.currentGameState);
    persistAndRenderGame();
}